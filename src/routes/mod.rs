...
fn has_preview_query(raw_query: &RawQuery) -> bool {
	raw_query
		.0
		.as_deref()
		.map(|query| url::form_urlencoded::parse(query.as_bytes()).any(|(key, _)| key == "preview"))
		.unwrap_or(false)
}
...
pub async fn search(
	...
	let search = if search.is_empty() { "*" } else { search };
	...
	top_docs
		...
		.map(|(_, doc_address): (DateTime, _)| -> Option<ImageInfo> {
			...
			let path = doc
				.get_first(database.image_schema.path)?
				.as_str()?
				.to_string();
			let tags: Vec<String> = doc
				.get_all(database.image_schema.tags)
...
pub async fn list_images(config: &Config, page: usize, page_size: usize) -> Option<Vec<String>> {
	Some(
		read_dir(&config.image_files_path)
			.ok()?
			.skip(page * page_size)
			.take(page_size)
			.flatten()
...
pub async fn get_image_viewer(
	...
	let tantivy_query = query_parser
		.parse_query(&format!("\"{}\"", query.id))
		.map_err(|e| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;

	let top_docs = searcher
		.search(&tantivy_query, &TopDocs::with_limit(1))
		.ok()
		.unwrap_or_default();
...
pub async fn delete_tag(
	...
	database
		.update_tags(&config.image_files_path, &query.id, &[query.tag], &[])
		.map_err(|e| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;

	get_image_viewer(
		Query(ImageViewerQuery { id: query.id }),
		State(BooruState { database, config }),
	)
	.await
}
...
pub async fn add_tag(
	...
	database
		.update_tags(&config.image_files_path, &query.id, &[], &[form.tag])
		.map_err(|e| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
...
pub async fn refresh_auto_tags(
	...
	let part = reqwest::multipart::Part::bytes(std::fs::read(&file_path).unwrap())
